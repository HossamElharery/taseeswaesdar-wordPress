...
class Page_Title extends Widget_Base {
	...
	protected function register_content_page_title_controls() {
		...
		$this->add_control(
			'block_page_title',
			[
				'label'   => __( 'Blog Page Title', 'header-footer-elementor' ),
				'type'    => Controls_Manager::TEXT,
				'dynamic' => [
					'active' => true,
				],
		...
		$this->add_control(
			'block_single_page_title',
			[
				'label'   => __( 'Blog Single Title', 'header-footer-elementor' ),
				'type'    => Controls_Manager::TEXT,
				'dynamic' => [
					'active' => true,
				],
	...
	protected function render() {
		...
		?>
			...
			<?php } ?>
			<<?php echo $settings['heading_tag']; ?> class="elementor-heading-title elementor-size-<?php echo $settings['size']; ?>">
				...
				if(is_home() && !is_front_page() || is_home() && is_front_page()){
					...
					if(!empty($bizgen_option['back_blog__title'])){
						echo esc_html($bizgen_option['back_blog__title']);
					}
					elseif(!empty($settings['block_page_title'])){
						echo wp_kses_post( $settings['block_page_title'] );
					} else {
						esc_html_e('Blog','builo');
					}
				}
				elseif(is_singular( 'post' )){
					$bizgen_option = get_option( 'bizgen_option' );
					if(!empty($bizgen_option['back_blog_single__title'])){
						echo esc_html($bizgen_option['back_blog_single__title']);
					}
					elseif(!empty($settings['block_single_page_title'])){
						echo wp_kses_post( $settings['block_single_page_title'] );
					} else {
						esc_html_e('Blog Single','builo');
					}					
				} 
				elseif(is_archive()){	
					if ( class_exists( 'WooCommerce' ) ) {	
						if(is_shop() || is_product_category() || is_product_tag()){	
						   woocommerce_page_title();
							...
							if (is_category()) {
						        echo $title = single_cat_title('', false);
						    } elseif (is_tag()) {
						        echo $title = single_tag_title('', false);
						    } elseif (is_author()) {
						        echo $title = get_the_author();
						    } elseif (is_tax()) {
						        echo $title = sprintf(__('%1$s'), single_term_title('', false));
						    } elseif (is_post_type_archive()) {
						        echo $title = post_type_archive_title('', false);
						    }
						}	
					}
					else{
				...
				elseif(is_search()){
					printf( __( 'Search Results for: %s', 'builo' ), '<span>' . get_search_query() . '</span>' );
				}
